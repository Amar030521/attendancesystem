...
router.get("/reports/monthly", async (req, res) => {
  try {
    ...
    const { data: cfgRows } = await supabase.from("config").select("key, value");
    const config = {}; (cfgRows || []).forEach(r => { config[r.key] = r.value; });
    const { data } = await supabase.from("attendance").select("*, users(name, designation, photo_url), clients(name), sites(name)").gte("date", `${month}-01`).lt("date", nextMonthStart(month)).order("date").order("labour_id").limit(10000);
...
router.get("/reports/client/:id", async (req, res) => {
  try {
    ...
    const { data } = await supabase.from("attendance").select("*, users(name, designation), sites(name)").eq("client_id", id).gte("date", start).lte("date", end).order("date").limit(10000);
...
router.get("/reports/site/:id", async (req, res) => {
  try {
    const { start, end, format } = req.query; const { id } = req.params;
    const { data } = await supabase.from("attendance").select("*, users(name, designation), clients(name)").eq("site_id", id).gte("date", start).lte("date", end).order("date").limit(10000);
